...
export const Route = createFileRoute("/auditing/new-audit/verify")({
...
type Row = {
  ...
  mrp: number;
  inventoryQty: number;
  countedQty: string; // input as string
};

// Try to read a value from a raw row using a list of possible header names (case/space insensitive)
...
function VerifyPage() {
  ...
  const totals = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      shortVal += shortQ * r.mrp;
      excessVal += excessQ * r.mrp;
    }
  ...
  const onFinish = async () => {
    setSaving(true);
    try {
      // Persist counted quantities back into audit_items.data
      const updates = rows
      ...
      await supabase.from("audits").update({ status: "verified" }).eq("id", id);
      toast.success("Inventory verification saved");
    ...
    <div className="relative min-h-screen bg-[#050b1e] text-white overflow-hidden">
      ...
      <div className="relative mx-auto max-w-[1400px] px-6 py-6 animate-fade-in">
        <header className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl px-6 py-4">
          <div>
            <h1 className="text-lg font-semibold">Verify Inventory</h1>
